...
export default function PracticeIndex() {
    ...
    return (
        <div className="h-full flex flex-col gap-2 overflow-hidden">
            ...
            <div className="flex-none grid grid-cols-12 gap-4 items-end border-b border-border pb-2">
                <div className="col-span-8">
                    <h1 className="text-2xl font-heading font-bold text-foreground tracking-tight">Practice Repository</h1>
                    <p className="text-sm text-muted-foreground mt-1 font-sans italic line-clamp-1">
            ...
            <div className="flex-1 min-h-0 flex flex-col gap-3">
                {/* Section Tabs */}
                <div className="flex items-center gap-1.5 overflow-x-auto p-1 no-scrollbar flex-none">
                    {sections.map(section => (
                        <button
                            key={section.id}
                            onClick={() => setActiveSection(section.id)}
                            className={cn(
                                "px-4 py-2 rounded-lg font-bold uppercase tracking-widest text-xs transition-all flex-shrink-0 border",
                ...
                <div className="flex-1 min-h-0 bg-secondary/5 border border-border/50 rounded-xl p-3 overflow-hidden">
                    {topicsWithQuestions.length > 0 ? (
                        <div className="h-full grid grid-cols-6 gap-2 content-start overflow-y-auto pr-1 no-scrollbar">
                            {topicsWithQuestions.map((topic) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        to={`/topic/${topic.id}`}
                                        className={cn(
                                            "academic-card flex flex-col justify-between p-2.5 transition-all duration-300",
                                            "hover:bg-primary/5 hover:border-primary/20 group relative overflow-hidden",
                                            "h-[88px]", // More compact height
                                            status === "in-progress" && "border-l-4 border-l-blue-500 pl-2",
